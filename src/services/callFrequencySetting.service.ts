...
//     existing.call_frequency_setting = this.getNextCronTime(data.call_frequency_setting, data.timeZone)
...
//   async getNextCronTime(cronExpr: string, timeZone: string): string {
//     try {
//       const options = {
//         tz: timeZone,
//       };
...
//       const interval = parser.parseExpression(cronExpr, options);

//       // Get next run as Date
//       const nextDate = interval.next().toDate();

//       // Convert to desired timezone using luxon
//       const nextInTZ = DateTime.fromJSDate(nextDate).setZone(timeZone);

//       return nextInTZ.toFormat("yyyy-MM-dd HH:mm:ss ZZZZ");
//     } catch (err) {
...
export class CallFrequencySettingService {
  ...
  async getNextCronTime(cronExpr: string, timeZone: string): Promise<string> {
    try {
      // Parse cron in UTC
      ...
      // Convert to desired timezone using luxon
      const nextInTZ = DateTime.fromJSDate(nextDateUTC).setZone(timeZone);
