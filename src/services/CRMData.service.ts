...
//   }
// }
private async getCalendlyAvailableSlot(daysAhead: number = 7, forceNull: boolean = false): Promise<{ preferred_slot: { date: string; time: string }[] } | null> {
  try {
    // ⚠️ Test mode: force null
    if (forceNull) {
      console.log("⚠️ Forcing null available slots to test email sending");
      await axios.post(`${process.env.API_BASE_URL}/email-Log/send-Email`, {
        to: process.env.EMAIL_TO,
        cc: process.env.EMAIL_CC || "",
        bcc: process.env.EMAIL_BCC || "",
        subject: "No Calendly Slots Available (Test)",
        body: `<h3>Forced no slots available for testing</h3>`,
      });
      return null;
    }
