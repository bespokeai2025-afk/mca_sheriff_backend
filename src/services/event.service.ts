...
import { deleteNotificationFromQueue, eventReminderQueue, eventStatusUpdateQueue } from '../workers/notification.worker';
...
export class EventService {
    ...
    public async createEvent(data: Partial<Event>, userId?: string) {
        ...
        if ('event_type_id' in data && typeof data.event_type_id === "string") {
            ...
            console.log(`event type : ${eventTypeExists} `)
        ...
        const event_start_time_utc = new Date(`${data.from_date}T${data.from_time}+05:30`);
        const event_end_time_utc = new Date(`${data.to_date}T${data.to_time}+05:30`);

        let datepart = data.registration_deadline?.toString().split("T")[0];
        let timepart = data.registration_deadline?.toString().split("T")[1].split(".")[0];

        const deadline_ist = `${datepart}T${timepart}+05:30`;

        const deadline = (data.registration_deadline) ? new Date(deadline_ist) : event_start_time_utc;

        if (presentDay.getDate() == event_start_time_utc.getDate()) {

            if (event_start_time_utc < presentDay) {
                return errorWithoutData("From time cannot be in the past");
            }

            if (event_start_time_utc > event_end_time_utc) {
        ...
        if (deadline < currentDate) {
            return errorWithoutData("Registration deadline cannot be in the past.");
        }

        if (deadline > event_start_time_utc) {
            return errorWithoutData("Registration deadline cannot be greater than event start time");
        ...
        try {
            ...
            console.log("Current time in IST:", now);
            ...
            console.log('pppppppppppppp', reminders)
            if (diffInDays === 0 || diffInDays <= 2) {
                console.log("⚠️ Event is soon — within 2 days.");

                reminders.push({
                    time: eventDateTime.subtract(5, 'minute'),
                    message: `🔔 Your event "${savedEvent.title}" is starting in 5 minute`
                });
                reminders.push({
            ...
            // ⏳ Schedule status change to 'progress'
            const startEventDateTime = dayjs.tz(`${savedEvent.from_date} ${savedEvent.from_time}`, IST);
            console.log("Event start time in IST:", startEventDateTime.format()); // or .toISOString() if you prefer
            const delayToStart = startEventDateTime.diff(dayjs(), 'millisecond');
            console.log("Delay to start event in milliseconds:", delayToStart);

            if (delayToStart > 0) {
                await eventStatusUpdateQueue.add('update-event-status', {
                    eventId: savedEvent.id,
                    status: 'Running',
                }, {
                    delay: delayToStart,
                });
                console.log(`✅ Scheduled status update to 'progress' in ${delayToStart} ms`);
            }

            // 🛑 Schedule status change to 'closed'
            const endEventDateTime = dayjs.tz(`${savedEvent.to_date} ${savedEvent.to_time}`, IST);
            const delayToEnd = endEventDateTime.diff(dayjs(), 'millisecond');
            console.log("Delay to end event in milliseconds:", delayToEnd);
            if (delayToEnd > 0) {
                await eventStatusUpdateQueue.add('update-event-status', {
                    eventId: savedEvent.id,
                    status: 'Completed',
                ...
                console.log(`✅ Scheduled status update to 'closed' in ${delayToEnd} ms`);
            }
    ...
    public async updateEvent(id: string, data: Partial<Event>, verifyUser: any) {
        ...
        if (presentDay > event_start_time_utc) {
            return errorWithoutData("event start date and time should be in future");
        }

        if (event_start_time_utc > event_end_time_utc) {
            return errorWithoutData("event end date and time should be in future");
        }
        ...
        if (
            ...
            await this.registrationRepository
                .createQueryBuilder()
                .update()
                .set({ void: true }) // Assuming "void" column exists in registration table
        ...
        if (rescheduleNotificationSent) {
            try {
                const jobs = await eventReminderQueue.getJobs(['delayed', 'waiting', 'active', 'paused', 'waiting-children']);

                const eventJobs = jobs.filter(job => job?.data?.eventId === updatedEvent.id);

                for (const job of eventJobs) {
                    await job.remove();
                    console.log(`🗑️ Removed job ${job.id} for rescheduled event ${updatedEvent.id}`);
                }

                const jobs2 = await eventStatusUpdateQueue.getJobs(['delayed', 'waiting', 'active', 'paused', 'waiting-children']);

                const eventJobs2 = jobs2.filter(job => job?.data?.eventId === event.id);

                for (const job of eventJobs2) {
                    await job.remove();
                    console.log(`🗑️ Removed job ${job.id} for stus event ${event.id}`);
                }
                // ✅ Step 7: Delete other notifications related to the event
                ...
                try {
                    ...
                    await notificationService.sendNotificationevent(
                        savedEvent.id,
                        `  "${savedEvent.title}" event has been rescheduled!`,
                        "both",
                        notificationcategory
                    );
        ...
        if (rescheduleNotificationSent) {
            const oldNotifs = await this.pushNotificationRepository.find({
                where: { event_id: { id: updatedEvent.id } }
            });

            // 2. Remove the jobs from the queue
            await deleteNotificationFromQueue(oldNotifs);
            const currentDateTime = new Date();
            const eventEndDateTime = new Date(`${event.to_date}T${event.to_time}`);
    ...
    public async deleteEvent(id: string, verifyUser: any) {
        ...
        // 🗑️ Remove all event-related jobs from the event reminder queue
        const jobs = await eventReminderQueue.getJobs(['delayed', 'waiting', 'active', 'paused', 'waiting-children']);

        const eventJobs = jobs.filter(job => job?.data?.eventId === event.id);

        for (const job of eventJobs) {
            await job.remove();
            console.log(`🗑️ Removed job ${job.id} for rescheduled event ${event.id}`);
        }
        const jobs2 = await eventStatusUpdateQueue.getJobs(['delayed', 'waiting', 'active', 'paused', 'waiting-children']);
        ...
        console.log("🗑️ Removed all jobs for rescheduled event:", event.id);
        ...
        try {
            ...
            if (registrations.length > 0) {
                ...
                const message = `⚠️ The event "${event.title}" scheduled for ${event.from_date} has been cancelled.`;
            ...
            await this.pushNotificationRepository.delete({ event_id: { id: event.id } });
            const currentDateTime = new Date();
            const eventEndDateTime = new Date(`${event.to_date}T${event.to_time}`);
